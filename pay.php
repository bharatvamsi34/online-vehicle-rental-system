...
<section class="">
	...
	<?php
		...
		if(isset($_POST['Hdate']))
		{
			...
			$id = $_SESSION['carid'];
			//echo $id;
			$selt = "SELECT * FROM client WHERE id_no = '$email'";
			$rst = $conn->query($selt);
			$rtemp = $rst->fetch_assoc();
			$cid=$rtemp['client_id'];
			$qry1 = "INSERT INTO `hire`(`hire_id`, `client_id`, `car_id`,`Hdate`, `status`) VALUES (null,$cid,'$id','$sdate','pending')";
			...
			<ul class="properties_list">
				...
				<?php
						if(isset($_POST['pay'])){
							...
							$qry = "UPDATE client SET mpesa = '$mpesa',status='Pending',car_id=$carid  WHERE id_no = '$id_no'";
